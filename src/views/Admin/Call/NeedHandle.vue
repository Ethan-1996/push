<template>
  <div class="settingContent">
    <el-breadcrumb separator="/" style="margin:20px 0 20px 40px">
      ...
      <el-breadcrumb-item>待处理电话</el-breadcrumb-item>
    </el-breadcrumb>

    <!-- 列表部分 -->
    <div class="phoneList">
      <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 550px">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-link type="primary" @click="dialogFormVisible = true;id = scope.row.has_one_phone_all.id">添加联系记录</el-link>
            <br />
            <el-link type="primary" @click="getCallPerson(scope.row.has_one_phone_all.id,scope.row.has_one_phone_all.nick)">获取联系记录</el-link>
            <br />
            <el-link type="primary" @click="backOpenSea(scope.row.id)">退回公海</el-link>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 添加 记录
    send(){
      addCallLog({al_id:this.id,type:this.radio},this.info).then(res => {
        if (res.data.code == 200) {
          this.$message({
            showClose: true,
            message: res.data.msg,
          ...
          for (let i = 0; i < this.tableData.length; i++) {
            //更新 成功后 更改试图
            if (this.id == this.tableData[i].has_one_phone_all.id) {
              this.tableData.splice(i, 1);
            }
          }
        } else {
