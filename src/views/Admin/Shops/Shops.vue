<template>
  <div class="settingContent">
    ...
    <el-table :data="tableData" border style="width: 1060px;margin:20px 0 40px 40px">
      ...
      <el-table-column prop="nick" label="店铺名称" width="160"></el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-link
            type="primary"
            @click="startBusiness(scope.row.id)"
            v-if="scope.row.state == 2"
          >启用</el-link>
          <el-link type="primary" @click="stopBusiness(scope.row.id)" v-else>禁用</el-link>
          <br />
          <el-link type="primary" @click="dialogVisible = true,text = scope.row.nick;businessEffect(scope.row.id)">效果展示</el-link>
        </template>
      </el-table-column>
    </el-table>
