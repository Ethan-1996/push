      ...
      <div class="info">
        ...
        <p class="logtext">管理员登录</p>
            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="0px" @submit.native.prevent>
                ...
                <el-form-item>
                    <div class="btnBox">
                        <el-button type="primary" @click="submitForm('ruleForm')" :disabled="flag" native-type="submit">登录</el-button>
...
export default {
    ...
    methods:{
        getPicCode(){  //获取图片流 需设置文件类型 且 只此一例  所以不在 公共api内设置
             axios.get(this.$header + '/admin/Login/sendPic',        //开发环境 反向代理
				 {
                    ...
                    checkUser(data).then(res => {
                        ...
                        if(res.data.code == 200){
                            this.$confirm('您已成功登录，是否保存用户信息用于下次登录?', '提示', {
                                ...
                                localStorage.setItem('accessTokenAdmin', res.data.data.app_key)
                                localStorage.setItem('user_idAdmin', res.data.data.admin_id)
                                localStorage.setItem('phoneAdmin', res.data.data.phone)
                                localStorage.setItem('role_list', res.data.data.role_list)
                                ...
                                sessionStorage.setItem('accessTokenAdmin',res.data.data.app_key);
                                sessionStorage.setItem('user_idAdmin',res.data.data.admin_id);
                                sessionStorage.setItem('phoneAdmin', res.data.data.phone)
                                sessionStorage.setItem('role_list', res.data.data.role_list)
                                this.$message({
                                    type: 'info',
