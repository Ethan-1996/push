<template>
    <div class="historyContent">
        ...
        <div class="goodsState">
            <div class="states">
                <el-link @click="getGoodsList(2,1,2,searchInfo)" :underline="false" :type="inlineType">当前在线商品</el-link>
                <el-link @click="getGoodsList(1,1,2,searchInfo)" :underline="false" :type="fuType">预告</el-link>
                <el-link @click="getGoodsList(4,1,2,searchInfo)" :underline="false" :type="noType">失效商品</el-link>
                <el-link @click="getGoodsList(3,1,2,searchInfo)" :underline="false" :type="endType">已结束</el-link>
            </div>
            <div class="search">
        ...
        <List :listInfo="listInfo" :info="info" type="2" :changeInfo="changeInfo"/>
...
export default {
    ...
    methods:{
        changeInfo(id,info){
            for (let i = 0; i < this.listInfo.length; i++) {
                if (this.listInfo[i].id == id) {
                    this.listInfo[i].state_to_platform_web = info
                }
        ...
        search(){
            ...
            this.getGoodsList(type,1,2,this.searchInfo)
            this.pageShow = false
            this.$nextTick(()=> {
                this.pageShow = true
            })
        },
        getGoodsList(state,page,upper_lower,keywords){
            switch (state) {
                case 1:
        ...
        handleCurrentChange(val) {    //切换 页码的函数
            ...
            this.getGoodsList(type,val,2,this.searchInfo)
        }
    }
